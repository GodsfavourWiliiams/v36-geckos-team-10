...
const itemClasses = `flex bg-white justify-center items-center gap-4 text-xl relative flex-1 bg-white 
                    ${tags === 'nav' ? 'h-16' : tags === 'img2' || tags === 'img' ? 'h-60' : 'h-32'}
...
const innerDivs = () => {
    ...
    do {
        myJsx.push(
        <div key={Math.random()} className={itemClasses}>
            {builderObject[tags] || 'notags'}{(divNum === 1 || divNum === 0) && 
